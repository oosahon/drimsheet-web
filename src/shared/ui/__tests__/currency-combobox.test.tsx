...
describe("CurrencySelect", () => {
  it("renders correctly with no initial value", () => {
    ...
    expect(
      screen.getByPlaceholderText("Select a currency"),
    ).toBeInTheDocument();
  });
  ...
  it("renders with an initial value and displays the logo", () => {
    ...
    const hasNgIcon = Array.from(logoDivs).some((div) => {
      ...
      return (
        style.backgroundImage.includes("ng.png") ||
        element.style.backgroundImage.includes("ng.png")
      );
    });
  ...
  it("filters currencies when typing by name", async () => {
    ...
    expect(
      screen.queryByRole("option", { name: /United States Dollar/i }),
  ...
  it("filters currencies when typing by code", async () => {
    ...
    expect(
      await screen.findByRole("option", { name: /United States Dollar/i }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("option", { name: /Nigerian Naira/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("calls onChange with empty string if value is cleared", async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<CurrencySelect label="Currency" value="USD" onChange={onChange} />);
